...
export class OllamaContentGenerator implements ContentGenerator {
  ...
  async generateContentStream(
    ...
    return (async function* () {
      try {
        // Convert Gemini format to Ollama format
        const prompt = self.convertToOllamaPrompt(request.contents);
        
        const response = await self.ollama.chat({
          model: self.model,
          messages: [{ role: 'user', content: prompt }],
          stream: true,
        });
        ...
        for await (const chunk of response) {
          if (chunk.message?.content) {
            yield self.convertToGeminiResponse(chunk);
          }
        }
      } catch (error) {
        throw new Error(`Ollama API error: ${error instanceof Error ? error.message : String(error)}`);
  ...
  async countTokens(request: CountTokensParameters): Promise<CountTokensResponse> {
    try {
      ...
      throw new Error(`Token counting error: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
